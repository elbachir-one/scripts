...
download_video () {
	...
	if [[ "$url" == *"youtube.com"* || "$url" == *"youtu.be"* ]]; then
		...
		if [[ "$url" == *"playlist"* || "$url" == *"channel"* ]]; then
			...
			yt-dlp --no-keep-video -f "137+bestaudio[ext=m4a]/299+bestaudio[ext=m4a]/bestvideo[height<=1080]+bestaudio[ext=m4a]" "$url" -o "%(playlist)s/%(title)s.%(ext)s"
		else
			# Use yt-dlp to check available formats for a single video
			yt-dlp --rm-cache-dir -F "$url"

			# Try to download video with format 137 or 299 and audio 140, fallback to best 1080p
			yt-dlp --no-keep-video -f "137+bestaudio[ext=m4a]/299+bestaudio[ext=m4a]/bestvideo[height<=1080]+bestaudio[ext=m4a]" "$url" -o "%(title)s.%(ext)s"
		fi

		# Get the downloaded filename
		downloaded_file=$(yt-dlp --get-filename -f "137+bestaudio[ext=m4a]/299+bestaudio[ext=m4a]/bestvideo[height<=1080]+bestaudio[ext=m4a]" "$url" -o "%(title)s.%(ext)s")
...
#        fi
#    else
#        echo "Invalid URL. Only YouTube and Twitch links are supported."
#    fi
#}
#
## Process each URL in the list if it's a file
